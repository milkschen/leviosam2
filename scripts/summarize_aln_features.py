...
def summarize_aln_features(
        ...
        for read in f:
            ...
            try:
                if tag:
                    ...
                    results.append(
                        read.query_alignment_length / read.query_length
                    )
            except:
                failed_cnt += 1
    ...
    if isize:
        print(
            f"not properly paird = {other_cnt} ({int(other_cnt / total * 100)}%)"
        )
...
if __name__ == "__main__":
    ...
    if sum(check_options) != 1:
        print(
            "Error: Only one out of the tag/mapq/isize/clipped_fraction options can be set"
        )
        print("[tag, mapq, isize, clipped_fraction]")
        print(check_options)
        exit(1)

    summarize_aln_features(
        fn_input=fn_input,
        tag=tag,
        mapq=mapq,
        isize=isize,
