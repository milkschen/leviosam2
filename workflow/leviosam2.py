...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--bgzip_binary", default="bgzip", type=str, help="Path to the bgzip executable"
    )
    parser.add_argument(
        "--gnu_time_binary",
        default="gtime",
    ...
    parser.add_argument(
        "--measure_time",
        action="store_true",
        help=("Activate to measure time with " "`--gnu_time_binary`"),
    )
    parser.add_argument(
        "--keep_tmp_files",
        action="store_true",
    ...
    parser.add_argument(
        "-g", "--lift_max_gap", type=int, help="[lift] Max chain gap size allowed"
    )
    parser.add_argument(
        "--lift_commit_min_mapq", type=int, help="[lift] Min MAPQ to commit"
    )
    parser.add_argument(
...
class Leviosam2Workflow:
    ...
    def validate_executable(cmd: str, lenient: bool = False) -> None:
        ...
        if not lenient and subprocess_out.returncode != 0:
            raise ValueError(
                f"`{cmd}` has a non-zero return code " f"`{subprocess_out.returncode}`"
            )
        else:
    ...
    def run_leviosam2(
        ...
        if self.dryrun:
            ...
            if (not self.forcerun) and self.fn_committed.is_file():
                print("[Info] Skip run_leviosam2 -- " f"{self.fn_committed} exists")
                return "skip"
    ...
    def run_sort_committed(
        ...
        if self.dryrun:
            ...
            if (not self.forcerun) and self.fn_committed_sorted.is_file():
                print(
                    "[Info] Skip run_sort_committed -- "
                    f"{self.fn_committed_sorted} exists"
                )
                return "skip"
            return subprocess.run([cmd], shell=True)

    def run_collate_pe(self) -> typing.Union[str, "subprocess.CompletedProcess[bytes]"]:
        ...
        if self.dryrun:
            ...
            if (
                (not self.forcerun)
                and self.fn_deferred_pe_fq1.is_file()
                and self.fn_deferred_pe_fq2.is_file()
            ):
                print(
                    "[Info] Skip run_collate_pe -- "
    ...
    def run_realign_deferred(
        ...
        if self.is_single_end:
            ...
            if self.aligner == "bowtie2":
                reads = f"-1 {self.fn_deferred_pe_fq1} " f"-2 {self.fn_deferred_pe_fq2}"
            else:
                reads = f"{self.fn_deferred_pe_fq1} " f"{self.fn_deferred_pe_fq2}"
    ...
    def run_refflow_merge_pe(
        ...
        if self.dryrun:
            ...
            if (not self.forcerun) and self.fn_deferred_reconciled.is_file():
                print(
                    "[Info] Skip run_refflow_merge_pe -- "
    ...
    def run_merge_and_index(
        ...
        cmd += f"{self.time_cmd}" f"{self.samtools} index {self.fn_final}"
        ...
        if self.dryrun:
            ...
            if (not self.forcerun) and self.fn_final.is_file():
                print(f"[Info] Skip run_merge_and_index -- {self.fn_final} exists")
                return "skip"
            return subprocess.run([cmd], shell=True)
    ...
    def run_bam_to_fastq_se(
        ...
        if self.dryrun:
            ...
            if (not self.forcerun) and self.fn_deferred_fq_se.is_file():
                print(
                    "[Info] Skip run_bam_to_fastq_se -- "
                    f"{self.fn_deferred_fq_se} exists"
                )
                return "skip"
            return subprocess.run([cmd], shell=True)

    def run_clean(self) -> typing.Union[str, "subprocess.CompletedProcess[bytes]"]:
        if self.dryrun:
    ...
    def _set_filenames(self):
        ...
        self.fn_deferred_pe_fq1 = o_dir / f"{o_prefix}-paired-deferred-R1.fq.gz"
        self.fn_deferred_pe_fq2 = o_dir / f"{o_prefix}-paired-deferred-R2.fq.gz"
        self.fn_deferred_pe = o_dir / f"{o_prefix}-paired-deferred.bam"
        self.fn_deferred_pe_sortn = o_dir / f"{o_prefix}-paired-deferred-sorted_n.bam"
        self.fn_deferred_realigned_pe = o_dir / f"{o_prefix}-paired-realigned.bam"
    ...
    def run_workflow(self):
        ...
        if self.is_single_end:
            ...
            self.run_realign_deferred(
                target_fasta_index=args.target_fasta_index, rg_string=args.read_group
            )
        else:
            self.run_collate_pe()
            self.run_realign_deferred(
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if args.measure_time:
            self.time_cmd = f"{self.gtime} -v " f"-ao {self.path_out_prefix}.time_log "
