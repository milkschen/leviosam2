...
class SamProcessing(unittest.TestCase):
    ...
    def tearDownClass(cls):
        for param in params:
            ...
            print(
                f'Cleaned up {param["out_prefix"]}.bam and {param["out_prefix"]}-RG.bam'
            )
    ...
    def test_picard_validate(self):
        for i, param in enumerate(params):
            with self.subTest(aln_param_idx=i):
                ...
                print(
                    f'[PASSED] Picard AddOrReplaceReadGroups & ValidateSamFile ({param["out_prefix"]}.bam)'
                )
...
PE_CHAIN_TESTS = {BT2_PE_CHM13: BT2_PE_H38, BWA_PE_CHM13: BWA_PE_H38}
SE_CHAIN_TESTS = {BT2_SE_CHM13: BT2_SE_H38, BWA_SE_CHM13: BWA_SE_H38}
...
class Chain(unittest.TestCase):
    def compare_aln(self, gold, result):
        if (
            gold.is_unmapped
            or result.is_unmapped
            or gold.is_supplementary
            or result.is_supplementary
        ):
            return
    ...
    def test_validate_long_reads(self):
        for fn in [MM2_HIFI_1, MM2_HIFI_2]:
            ...
            stdout, stderr = process.communicate()
            process2 = subprocess.Popen(
                ["picard", f"ValidateSamFile I={fn}-lifted.bam MODE= VERBOSE"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            stdout, stderr = process2.communicate()
            self.assertNotIn("ERROR", stderr.decode(), msg='Check "No errors found"')
