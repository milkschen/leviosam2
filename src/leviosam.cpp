...
void serialize_run(lift_opts args) {
  if (args.outpre == "") {
    std::cerr
        << "[E::serialize_run] No output prefix specified. Please set `-p`.\n";
  ...
  if (args.vcf_fname != "") {
    ...
    std::cerr << "[E::serialize_run] Cannot build a levioSAM index. Please set "
                 "-v or -c properly\n";
    print_serialize_help_msg();
    exit(1);
  }
...
void read_and_lift(T *lift_map, std::mutex *mutex_fread,
  ...
  while (read >= 0) {
    ...
    for (int i = 0; i < num_actual_reads; i++) {
      ...
      if (!args.md_flag) {
        // strip MD and NM tags if `md_flag` not set bc liftover invalidates
        // them
      ...
      if (nm_ptr != NULL && !(aln_vec[i]->core.flag & BAM_FUNMAP)) {
        ...
        std::string ref_seq = ref.substr(
            aln_vec[i]->core.pos, bam_cigar2rlen(aln_vec[i]->core.n_cigar,
...
void lift_run(lift_opts args) {
  chain::ChainMap chain_map = [&] {
    if (args.chainmap_fname != "") {
      ...
      if (args.length_map.size() == 0) {
        std::cerr << "[E::lift_run] No length map is found. Please set -F "
                     "properly.\n";
        print_serialize_help_msg();
        exit(1);
      }
  ...
  lift::LiftMap lift_map = [&] {
    if (args.lift_fname != "") {
      ...
      std::cerr << "[I::lift_run] Building levioSAM index...";
      return lift::LiftMap(lift::lift_from_vcf(args.vcf_fname, args.sample,
                                               args.haplotype, args.name_map,
                                               args.length_map));
  ...
  if (args.split_rules.size() != 0) {
    wd.init(args.outpre, args.split_rules, args.out_format, hdr_orig, hdr,
            args.bed_defer_source, args.bed_defer_dest, args.bed_commit_source,
            args.bed_commit_dest);
  }
  ...
  for (int j = 0; j < args.threads; j++) {
    if (args.chain_fname == "" && args.chainmap_fname == "") {
      threads.push_back(std::thread(
          read_and_lift<lift::LiftMap>, &lift_map, &mutex_fread, &mutex_fwrite,
          sam_fp, out_sam_fp, hdr_orig, hdr, &ref_dict, &wd, args));
    } else {
      threads.push_back(std::thread(read_and_lift<chain::ChainMap>, &chain_map,
                                    &mutex_fread, &mutex_fwrite, sam_fp,
                                    out_sam_fp, hdr_orig, hdr, &ref_dict, &wd,
                                    args));
    }
  }
  for (int j = 0; j < args.threads; j++) {
...
lift::LiftMap lift::lift_from_vcf(std::string fname, std::string sample,
  ...
  if (fname == "" && sample == "") {
    std::cerr
        << "[E::lift_from_vcf] Vcf file name and sample name are required.\n";
    print_serialize_help_msg();
    exit(1);
  }
...
void print_lift_help_msg() {
  ...
  std::cerr << "                   Each thread queries <-T> reads, lifts, and "
               "writes.\n";
  ...
  std::cerr << "                       * isize         INT   Max TLEN/isize to "
  ...
  std::cerr << "           -r string Path to a BED file (source coordinates). "
...
int main(int argc, char **argv) {
  ...
  for (auto &r : args.split_rules) {
    if (r.first == "hdist") {
      if (args.ref_name == "") {
        std::cerr
            << "[E::main] Option `-f` must be set when `-S hdist` is used\n";
        print_lift_help_msg();
        exit(1);
      }
    }
  ...
  if (args.split_rules.size() == 0) {
    if (args.bed_defer_source.get_fn() != "" ||
      ...
      std::cerr
          << "[E::main] `-S` should be set if any among `-d/D/r/R` is set.\n";
      exit(1);
    }
  }
  if (args.realign_yaml != "") {
