...
void read_and_lift(
    ...
    while (read >= 0){
        ...
        for (int i = 0; i < num_actual_reads; i++){
            ...
            if (!args.md_flag) {
                // strip MD and NM tags if `md_flag` not set bc liftover invalidates them
            ...
            if (nm_ptr != NULL && !(aln_vec[i]->core.flag & BAM_FUNMAP)) {
                ...
                std::string ref_seq = ref.substr(
                    aln_vec[i]->core.pos,
                    bam_cigar2rlen(aln_vec[i]->core.n_cigar, bam_get_cigar(aln_vec[i])));
                ...
                if (Aln::align_ksw2(ref_seq.data(), q_seq.data(), args.aln_opts,
                    ...
                    bool first_seg = (aln_vec[i]->core.flag & BAM_FPAIRED)?
                        (aln_vec[i]->core.flag & BAM_FREAD1)? true : false: true;
