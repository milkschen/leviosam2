...
public:
  ...
  Lift(Lift &&rhs)
      : ins(std::move(rhs.ins)), del(std::move(rhs.del)),
  ...
  std::vector<uint32_t> lift_cigar_core(bam1_t *b) {
    ...
    while (y < iters) {
      ...
      if (del[x]) { // skip ahead in alt2ref cigar
        ...
        // we separate this from the D branch in case we have to do something
        // special ie. spliced alignments
        if (!ins[x]) {
          if (new_cigar_ops.empty()) {
            new_cigar_ops.push_back(BAM_CREF_SKIP);
  ...
  /* Convert a CIGAR string in an s2 alignment to the corresponding CIGAR string
   * in the s1 alignment
   *
...
public:
  ...
  // move constructor
  LiftMap(LiftMap &&rhs)
      : lmap(std::move(rhs.lmap)), s1_map(std::move(rhs.s1_map)),
  ...
  LiftMap(vcfFile *fp, bcf_hdr_t *hdr, std::string sample_name,
    ...
    if (no_sample)
      fprintf(stderr, "no sample given, assuming GT=1 for all variants\n");
    ...
    while (!bcf_read(fp, hdr, rec)) {
      ...
      if (rec->rid != rid) {
        ...
        if (l == 0) {
          if (!ls.size()) {
            die("contig length not specified in vcf, nor was it provided by "
                "user!\n");
          } else {
      ...
      if (no_sample || (ngt > 0 && !bcf_gt_is_missing(gt_arr[hap]) &&
        ...
        // For some variant types POS+REF refer to the base *before* the event;
        // in such case set trim_beg
        int trim_beg = 0;
        ...
        if (rec->pos <= tppos) {
          int overlap = 0;
          if (rec->pos < tppos || !trim_beg || var_len == 0 || prev_is_ins)
            overlap = 1;
          if (overlap) {
            if (verbose)
        ...
        // at this point, ppos and x should point to *end* of last variant in s1
        // & s2 alignment, resp.
        x += (rec->pos - ppos); // x should now be pointing to *start* of
                                // current variant wrt alignment
  ...
  // sequence input: sequence name, position within s2 sequence output: position
  // within s1 sequence if liftover is not defined, then returns the original
  // position
    ...
    if (c.flag & BAM_FPAIRED) {
      ...
      if (c.flag & BAM_FUNMAP) {
        ...
        if (c.flag & BAM_FMUNMAP) {
          ...
          aln->core.mpos = mpos;
          int isize =
              (c.isize == 0) ? 0 : c.isize + (mpos - c.mpos) - (pos - c.pos);
  ...
  void add_names(std::string n1, std::string n2, int i) {
    ...
    if (n1 != n2) {
      if (name_map.find(n2) == name_map.end())
        ...
        die("non one-to-one mappings not supported yet! Or there are identical "
