...
TEST(ChainTest, LiftBamInReversedRegion) {
  ...
  while (sam_read1(sam_fp, hdr, aln) == 0) {
    ...
    EXPECT_EQ(cmap.lift_segment(aln, hdr, hdr_gold, true, dest_contig), true);
...
TEST(ChainTest, LiftCigar5) {
  ...
  kstring_t str;
  std::string record = "16M_3S13M	256	chr1	687455	42	"
                       "16M	*	0	0	*	*";
  str.s = (char *)record.c_str();
  str.l = record.length();
  str.m = kstr_get_m(str.l);
...
TEST(ChainTest, LiftExtendedCigar1) {
  ...
  // CIGAR should not change
  kstring_t str;
  std::string record = "7=13D6=1X6=_7M13D13M	0	chr1	674850	"
                       "42	7=13D6=1X6=	*	0	0	"
...
TEST(ChainTest, LiftExtendedCigarReverse2) {
  ...
  kstring_t str;
  std::string record =
      "10=1X5=_10M1D6M	0	chr1	145331505	42	10=1X5=	"
      "*	0	0	ATTACATTCCATTCCA	~~~~~~~~~~~~~~~~";
  str.s = (char *)record.c_str();
  str.l = record.length();
  str.m = kstr_get_m(str.l);
