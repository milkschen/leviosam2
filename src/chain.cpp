...
int ChainMap::lift_cigar(const std::string &contig, bam1_t *aln,
  ...
  // format, but only outputs in the traditional format, where both matches and
  // mismatches are represented using the `M` operator.
      ...
      while (next_q_offset > next_bp && next_bp >= query_offset) {
        ...
        if (verbose >= VERBOSE_DEV) {
          std::cerr << "  first half: " << first_half_len
                    << bam_cigar_opchr(cigar_op) << " (query_offset -> "
                    << query_offset << ")\n";
          if (diff > 0)
...
std::vector<uint32_t> ChainMap::lift_cigar_core(
  ...
  if (num_sclip_start > 0) {
    sclip_cigar_front(cigar, aln->core.n_cigar, num_sclip_start, new_cigar, idx,
                      query_offset);
  }
  for (auto i = idx; i < aln->core.n_cigar; i++) {
    ...
    // and mismatches are represented using the `M` operator. if (cigar_op ==
    // BAM_CEQUAL || cigar_op == BAM_CDIFF)
    //     cigar_op = BAM_CMATCH;
    ...
    if (start_sidx == end_sidx) {
      ...
      // Lift one cigar run
      lift_cigar_core_one_run(new_cigar, break_points, cigar_op_len, cigar_op,
...
bool ChainMap::lift_segment(bam1_t *aln, sam_hdr_t *hdr_source,
  ...
  if (start_sidx != end_sidx) {
    ...
    if (!check_multi_intvl_legality(source_contig, aln, start_sidx, end_sidx,
                                    allowed_intvl_gaps)) {
      return false;
    }
  }
  ...
  if (c->pos < 0) {
    ...
    // Invalid mate positions can be benign since there's no CIGAR information
    // to add SOFT_CLIP based in the beginning. We report these as INFO but not
    // WARNING
    if (verbose >= VERBOSE_INFO) {
      std::cerr << "[I::chain::lift_segment] The mate of read "
                << bam_get_qname(aln) << " is lifted to a negative position ("
                << c->mpos << ")\n";
    }
    c->mpos = 0;
    return false;
...
hts_pos_t ChainMap::lift_pos(const std::string &contig, const hts_pos_t &pos,
  ...
  if (left) {
    int32_t num_sclip_start = get_num_clipped(pos, true, contig, sidx, eidx);
...
void sclip_cigar_front(uint32_t *cigar, const uint32_t &n_cigar, int len_clip,
  ...
  for (auto i = idx; i < n_cigar; i++) {
    ...
    // operator
    if (bam_cigar_type(cigar_op) & 1) {
      // If a CIGAR OP is longer than #clipped, append clipped bases to the
      // updated CIGAR and truncate the original CIGAR for future usage.
  ...
  for (int i = cigar.size() - 1; i >= 0; i--) {
    ...
    if (bam_cigar_type(cigar_op) & 1) {
      if (cigar_op_len >= remaining_lc) {
        ...
        std::cerr << "[W::chain::sclip_cigar_back] Unexpected OP during "
                     "back_clipping: "
                  << cigar_op << "\n";
      }
    }
...
std::queue<std::tuple<int32_t, int32_t>>
ChainMap::get_bp(const std::string &contig, const bam1_core_t *const c,
                 const int &start_sidx, const int &end_sidx) {
  std::queue<std::tuple<int32_t, int32_t>> break_points;
  for (auto i = start_sidx; i <= end_sidx; i++) {
    ...
    if (interval_map[contig][i].strand) { // forward
      diff =
          interval_map[contig][i + 1].offset - interval_map[contig][i].offset;
    } else { // reverse
...
bool ChainMap::check_multi_intvl_legality(const std::string &s, bam1_t *aln,
  ...
  for (auto j = start_sidx; j < end_sidx; j++) {
    if (interval_map[s][j].strand != interval_map[s][j + 1].strand) {
      if (verbose >= VERBOSE_INFO) {
        std::cerr
            << "[I::chain::check_multi_intvl_legality] Set "
            << bam_get_qname(aln)
    ...
    if (std::abs(interval_map[s][j + 1].offset - interval_map[s][j].offset) >
        allowed_intvl_gaps) {
      if (verbose >= VERBOSE_INFO) {
        std::cerr << "[I::chain::check_multi_intvl_legality] Set "
                  << bam_get_qname(aln) << " to unmapped -- >=1 chain gaps > "
                  << allowed_intvl_gaps << "\n";
      }
      return false;
    }
