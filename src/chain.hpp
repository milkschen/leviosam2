...
public:
  ...
  int lift_cigar(const std::string &contig, bam1_t *aln, const int &start_sidx,
                 const int &end_sidx, int num_sclip_start, int num_sclip_end);
  std::vector<uint32_t> lift_cigar_core(const std::string &contig, bam1_t *aln,
